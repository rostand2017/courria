...
class ClientRepository extends \Doctrine\ORM\EntityRepository
{
    public function countByDateAndName($nbPage, $page, $begin=null, $end=null, $search=null){
        $page = (int) $page;
        $nbPage = (int) $nbPage;
        $sql="SELECT COUNT(*) FROM client c ";

        if($search && !$begin)
            $sql .= " WHERE c.nom LIKE :search OR c.prenom LIKE :search ";
        else if($search && $begin)
            $sql .= " WHERE c.createdat BETWEEN :begin AND :end AND (c.nom LIKE :search OR c.prenom LIKE :search)";
        else if(!$search && $begin)
