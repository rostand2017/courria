...
class InfluencerRepository extends \Doctrine\ORM\EntityRepository
{
    public function countByDateAndName($nbPage, $page, $begin=null, $end=null, $search=null){
        $page = (int) $page;
        $nbPage = (int) $nbPage;
        $sql="SELECT COUNT(*) FROM influencer c ";

        if($search && !$begin)
            $sql .= " WHERE c.name LIKE :search OR c.email LIKE :search ";
        else if($search && $begin)
            $sql .= " WHERE c.createdat BETWEEN :begin AND :end AND (c.name LIKE :search OR c.email LIKE :search)";
        else if(!$search && $begin)
    ...
    public function getByDateAndName($limit, $offset, $begin=null, $end=null, $search=null){
        ...
        $sql="SELECT * FROM influencer c";

        if($search && !$begin)
            $sql .= " WHERE c.name LIKE :search OR c.email LIKE :search ";
        else if($search && $begin)
            $sql .= " WHERE c.createdat BETWEEN :begin AND :end AND (c.nom LIKE :search OR c.name LIKE :email )";
        else if(!$search && $begin)
            $sql .= " WHERE c.createdat BETWEEN :begin AND :end ";
